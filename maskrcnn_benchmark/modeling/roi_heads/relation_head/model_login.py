...
from .login_utils import Anchor, masking, CoordConv, RelationalEmbedding
from .login_local_interact import MultiHeadAttention, CBAM, NonLocalBlock, AWAttention
from .login_global_interact import get_adjacency_mat, GCN, GAT, SpGAT, AGAIN
...
class LOGIN(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        # if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_COORD_CONV:
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.REDUCE_DIM:
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_TYPE == "self_att":
            self.att = MultiHeadAttention(num_heads=8, d_model=self.dim*3) if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_ALL_AT_ONCE \
                else nn.ModuleList([MultiHeadAttention(num_heads=8, d_model=self.dim) for _ in range(3)])
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_TYPE == "cbam":
            self.att = CBAM(in_channels=self.out_dim*3, reduction_ratio=8, kernel_size=3) if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_ALL_AT_ONCE \
                else nn.ModuleList([CBAM(in_channels=self.out_dim, reduction_ratio=8, kernel_size=3) for _ in range(3)])
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_TYPE == "non_local":
            self.att = NonLocalBlock(in_channels=self.out_dim, reduction_ratio=2, subsample=True, use_bn=True, dim=3) if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_ALL_AT_ONCE \
                else nn.ModuleList([NonLocalBlock(in_channels=self.out_dim, reduction_ratio=2, use_bn=True, dim=2) for _ in range(3)])
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_TYPE == "awa":
            self.att = AWAttention(channels=self.out_dim*3, height=resolution*3, width=resolution*3, hidden_dim=self.dim*3, pool='avg', residual=True) if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_ALL_AT_ONCE \
        ...
        self.edge2edge = cfg.MODEL.ROI_RELATION_HEAD.LOGIN.EDGE2EDGE
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.GRAPH_INTERACT_MODULE == "gcn":
            self.graph_interact = GCN(num_layers=cfg.MODEL.ROI_RELATION_HEAD.LOGIN.NUM_GIN_LAYERS, dim=self.dim, dropout=0.1, residual=True)
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.GRAPH_INTERACT_MODULE == 'gat':
            self.graph_interact = GAT(dim=self.dim, num_heads=8, concat=True, dropout=0.1)
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.GRAPH_INTERACT_MODULE == 'again':
            self.graph_interact = AGAIN(num_layers=cfg.MODEL.ROI_RELATION_HEAD.LOGIN.NUM_GIN_LAYERS, dim=self.dim, num_heads=8, concat=True, residual=False, dropout=0.1)
        elif self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.GRAPH_INTERACT_MODULE == 'self_att':
    ...
    def _att(self, rel_features):
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.ATT_ALL_AT_ONCE:
    ...
    def forward(self, roi_features, proposals, rel_features, rel_pair_idxs, logger=None):
        ...
        rel_features: tensor(182x256x7x7) (where, 182=72+110)
        ...
        if self.mode == 'predcls':
            ...
            obj_dists = torch.cat([proposal.get_field("predict_logits").detach() for proposal in proposals], 0) # 20x151 "predict_logits"
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_ATT:
            rel_features = self._att(rel_features)

        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.FLATTEN:
        ...
        # 3. compose
        if self.cfg.MODEL.ROI_RELATION_HEAD.POOL_SBJ_OBJ or self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_MASKING:
            rel_features = self._compose(rel_features, compose_type=self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.COMPOSE_TYPE)
        else:
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_GIN:
            if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.GRAPH_INTERACT_MODULE == "self_att":
        ...
        # atrract & repel
        # if self.training:
        #     if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_BI_ATT_REP_LOSS:
        ...
        #      # repulsive loss 
        #     if self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.USE_REPULSIVE_LOSS:
        #         for rel_pair_idx in rel_pair_idxs:
        #             num_rels = rel_pair_idx.shape[0]
        ...
        #                 margin = torch.tensor(self.cfg.MODEL.ROI_RELATION_HEAD.LOGIN.MARGIN, device=pdist.device)
