...
class _Collection_Unit(nn.Module):
    def __init__(self, dim_in, dim_out): # 1024, 1024
        super(_Collection_Unit, self).__init__()
        self.fc = nn.Linear(dim_in, dim_out, bias=True) # 1024, 1024
...
class _GraphConvolutionLayer_Collect(nn.Module):
    ...
    """
    def __init__(self, dim_obj, dim_rel): # 1024, 1024
        super(_GraphConvolutionLayer_Collect, self).__init__()
        self.collect_units = nn.ModuleList()
        self.collect_units.append(_Collection_Unit(dim_rel, dim_obj)) # 0. from rel to subj
        self.collect_units.append(_Collection_Unit(dim_rel, dim_obj)) # 1. from rel to obj
        self.collect_units.append(_Collection_Unit(dim_obj, dim_rel)) # 2. from subj to rel
        self.collect_units.append(_Collection_Unit(dim_obj, dim_rel)) # 3. from object to rel
        self.collect_units.append(_Collection_Unit(dim_obj, dim_obj)) # 4. from obj to obj
...
class _GraphConvolutionLayer_Update(nn.Module):
    ...
    def __init__(self, dim_obj, dim_rel):
        ...
        self.update_units.append(_Update_Unit(dim_obj)) # 0. obj from others
        self.update_units.append(_Update_Unit(dim_rel)) # 1. rel from others
