...
class GRCNN(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.att = AttentionGate(in_channels=2048, reduction_ratio=128, kernel_size=3)
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        # x_obj = torch.cat([self.avgpool(self.att(proposal.get_field("raw_features").detach())) for proposal in proposals], 0) # Nx2048x7x7
        ...
        x_pred, _, loss_multi, loss_pos = self.pred_feature_extractor(features, proposals, proposal_pairs)
        ...
        obj_class_logits = self.obj_predictor((obj_feats[0] + obj_feats[-1]).unsqueeze(2).unsqueeze(3))
        pred_class_logits = self.pred_predictor((pred_feats[0] + pred_feats[-1]).unsqueeze(2).unsqueeze(3))
        ...
        obj_class_logits = obj_scores[0] + obj_scores[-1]
        pred_class_logits = pred_scores[0] + pred_scores[-1]
        ...
        return (x_pred), obj_class_logits, pred_class_logits, obj_class_labels, rel_inds, loss_multi, loss_pos
