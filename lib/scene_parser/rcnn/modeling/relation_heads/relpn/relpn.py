...
from lib.scene_parser.rcnn.modeling.relation_heads.glove import load_embeddings
...
class RelPN(nn.Module):
    def __init__(
        ...
        # self.word_embeddings = load_embeddings(dim=100)
    ...
    def _relpnsample_train(self, proposals, targets):
        ...
        for img_idx, (proposals_per_image, pos_inds_img, neg_inds_img) in \
            ...
            obj_features = proposals_per_image.get_field('features').squeeze()
            obj_logits = proposals_per_image.get_field('logits') # Nx151
            # obj_words = torch.mm(obj_logits, self.word_embeddings.to(obj_logits.device)) # (Nx151) x (151x300) = Nx300
            obj_bboxes = proposals_per_image.bbox
            relness = self.relevance(obj_features, obj_logits, obj_bboxes, proposals_per_image.size)
            # relness = self.relevance(obj_features, obj_words, obj_logits, obj_bboxes, proposals_per_image.size)
